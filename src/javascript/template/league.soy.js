...
ffc.template.league.userTable = function(opt_data, opt_ignored) {
  ...
  if (userListLen36 > 0) {
    for (var userIndex36 = 0; userIndex36 < userListLen36; userIndex36++) {
      ...
      output += '<tr><td><img src="' + soy.$$escapeHtml(userData36.pic) + '" alt="' + soy.$$escapeHtml(userData36.name) + '" width="20" height="20"></td><td><a href="/u/' + soy.$$escapeHtml(userData36.name) + '">' + soy.$$escapeHtml(userData36.name) + '</a></td><td><button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-remove"></span> Remove</button></td></tr>';
    }
  } else {
